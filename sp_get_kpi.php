...
for ($i=0; $i<count($arrKPI);$i++){
	...
	for($m=1+$oBudget->offset;$m<=$ytd;$m++){
		...
		$sql[] = "INSERT INTO budget.reg_sales (pc,activity,unit,customer,`{$month}`,source,scenario,active,posted,kpi,sales, bdv, bo)
					SELECT jobProfitID, @prtID, @unit, cntID, {$arrKPI[$i]['kpi']} as '{$month}', 'Actual', @scenario, 1,1,1, cntUserID, usrProfitID, IFNULL(jobGDSBusinessOwnerID,714)
					FROM nlogjc.tbl_job
